...
-   **Local Vector Storage**: Utilizes **Milvus Lite** for efficient, standalone vector data management. This allows for RAG (Retrieval-Augmented Generation) and history search capabilities without external dependencies.
...
-   **Vector Database Configuration**:
    -   Data is stored in `db/milvus/data/milvus.db`.
    -   Configuration file: `config/storage_config.py`.
    -   You can change the storage path by modifying `MILVUS_DATA_PATH` in the config file.
...
-   **Milvus Connection Error**: Ensure `milvus-lite` and `pymilvus` are correctly installed. Check write permissions for the `db/` directory.
