...
class AudioEmbeddingServer(BaseServer, Embeddings):
    ...
    def init_model(self, params: dict):
        ...
        if not arch or arch not in self.model_list:
             print("Warning: No valid model configuration found for Audio Embedding. Skipping initialization.")
             return

        model_path = self.model_list[arch][model_name]["model_path"]
        backend, impl = resolve_model_config("audio_embedding", arch, model_name)
    ...
    def reload_model(self, infer_arch: str = "", device: str = "", model_name: str = "", model_version: str = "", default: bool = False):
        ...
        backend, impl = resolve_model_config("audio_embedding", infer_arch, model_name)
        self.engine = ModelEngine(
            model_type="audio_embedding",
