...
class QwenOmiTransformerModel(BaseModel):
    def load_model(self, model_path: str, device: str, backend: str = "transformer", **kwargs):
        # We can use TorchBackend here partially, but QwenOmni needs specific model class
        # TorchBackend uses AutoModelForCausalLM by default.
        # We can either extend TorchBackend or just keep using direct Qwen2_5OmniModel here for now,
        # but structured properly.
        # Since Qwen2_5OmniModel is specialized, let's keep it explicit but wrapped.
        
        # Note: If we really want to use backend pattern, we'd need to allow backend to load custom model classes.
        # For now, let's just accept backend arg to satisfy interface.
    ...
    def chat(self, history, max_tokens, temperature, top_p, slider_context_times, return_audio=True):
        ...
        if return_audio:
            if not os.path.exists(CACHE_AUDIO_PATH):
                os.makedirs(CACHE_AUDIO_PATH, exist_ok=True)
