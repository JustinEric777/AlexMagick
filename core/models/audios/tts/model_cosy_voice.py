...
class CosyVoiceModel(BaseModel):
    ...
    def inference(self, texts: [], sample_wav: str = None):
        if sample_wav is None:
            try:
                import cosyvoice
                base_dir = os.path.dirname(cosyvoice.__file__)
                candidates = [
                    os.path.join(base_dir, "examples", "zero_shot_prompt.wav"),
                    os.path.join(base_dir, "assets", "zero_shot_prompt.wav"),
                ]
                sample_wav = next((p for p in candidates if os.path.exists(p)), None)
            except Exception:
                sample_wav = None
            if sample_wav is None:
                raise ValueError("CosyVoice 需要提供参考音频 sample_wav；请传入有效 WAV 路径。")
