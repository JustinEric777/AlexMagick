...
def create_ui(args: dict):
    def render_main():
        ...
        def update_results(original_text, generated_audio, metric_value):
            ...
            audio = create_audio_player(generated_audio)
            new_row = [original_text, audio, metric_value.strip()]
            items.append(new_row)
            return items

        generate_bt.click(tts.generate, inputs=[text_input], outputs=[audio_output, metric], queue=False).then(
    ...
    create_standard_page(tts, args, render_main, tab_label="TTS Model", tab_id="tts_tab")
