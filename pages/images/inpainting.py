...
def create_ui(args: dict):
    ...
    def render_main():
        with gr.Row():
            with gr.Column(scale=2):
                ...
                with gr.Row():
                    components['image_base'] = gr.Image(type="filepath", width=200, label="Base Image", sources=["upload"])
                    components['image_mask'] = gr.Image(type="filepath", width=200, label="Mask Image", sources=["upload"])
                with gr.Row():
        ...
        with gr.Row():
            with gr.Column(scale=4):
                gr.Examples(
                    ...
                    inputs=[components['image_base'], components['image_mask'], components['positive_prompt'], components['negative_prompt']],
                )
    ...
    create_standard_page(inpainting, args, render_main, render_params_ui=render_params, tab_label="Inpainting Model", tab_id="image_inpainting_tab")
    ...
    def update_results(image_input_value, image_mask_value, positive_prompt_value, negative_prompt_value, result_image_value, metric_value):
        ...
        mask_image = create_image_html(image_mask_value)
    ...
    components['generate_bt'].click(
        generate, 
        inputs=[
            components['image_base'], components['image_mask'], components['positive_prompt'], components['negative_prompt'], 
        ...
        update_results, 
        inputs=[components['image_base'], components['image_mask'], components['positive_prompt'], components['negative_prompt'], components['image_output'], components['metric']], 
