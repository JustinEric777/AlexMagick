...
def create_ui(args: dict):
    with gr.Tab(label="InPainting Model", id="image_inpainting_tab") as image_inpainting_tab:
        ...
        generate_bt.click(inpainting.generate, inputs=[positive_prompt, negative_prompt, seed, guidance_scale, num_inference_steps, width, height, model_name], outputs=[image_output, metric], queue=False).then(
